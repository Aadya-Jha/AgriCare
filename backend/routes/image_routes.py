...
def process_image_with_matlab(image_path, output_path):
    ...
    try:
        ...
        if hasattr(result, '_data'):
            ...
            # Extract vegetation indices if available
            if 'ndvi_stats' in result:
                ndvi_stats = result['ndvi_stats']
                python_result['ndvi'] = float(ndvi_stats['mean']) if 'mean' in ndvi_stats else 0.7
            
            if 'savi_stats' in result:
                savi_stats = result['savi_stats']
                python_result['savi'] = float(savi_stats['mean']) if 'mean' in savi_stats else 0.6
                
            if 'evi_stats' in result:
                evi_stats = result['evi_stats']
                python_result['evi'] = float(evi_stats['mean']) if 'mean' in evi_stats else 0.5
                
            # Add additional metrics
            python_result['soil_brightness'] = float(result.get('soil_brightness_mean', 0.3))
            python_result['soil_moisture'] = float(result.get('soil_moisture_mean', 0.25))
            python_result['red_edge_position'] = 735.0  # Default if not available
            
            # Health assessment based on indices
            ndvi_val = python_result.get('ndvi', 0.7)
            if ndvi_val > 0.7:
            ...
            python_result['health_assessment'] = {
                'overall_health': health_status,
                'stress_indicators': stress_level,
                'vegetation_coverage': f"{int(ndvi_val * 100)}%"
            }
            
            # Check if processing result files exist
            results_file = os.path.join(output_path, 'processing_results.json')
            if os.path.exists(results_file):
                try:
                    with open(results_file, 'r') as f:
                        file_results = json.load(f)
                        python_result.update(file_results)
                except Exception as e:
                    print(f"Error reading MATLAB results file: {e}")
