...
import apiService, { 
  ...
  TrendData, 
  SensorDataResponse, 
  HyperspectralPredictionsResponse, 
  LocationPrediction, 
  HyperspectralTrainingResult, 
  HyperspectralModelInfo 
} from '../services/api';
...
// ==================== HYPERSPECTRAL ANALYSIS HOOKS ====================

export function useHyperspectralPredictions(): ApiHook<HyperspectralPredictionsResponse> {
  return useApiCall(() => apiService.getHyperspectralPredictions());
}

export function useLocationPrediction(locationName: string): ApiHook<LocationPrediction> {
  return useApiCall(() => apiService.getLocationPrediction(locationName), [locationName]);
}

export function useHyperspectralModelInfo(): ApiHook<HyperspectralModelInfo> {
  return useApiCall(() => apiService.getHyperspectralModelInfo());
}

export function useHyperspectralTraining(trigger: boolean = false): ApiHook<HyperspectralTrainingResult> & { startTraining: () => void } {
  const [shouldTrain, setShouldTrain] = useState(trigger);
  
  const apiHook = useApiCall(
    () => {
      if (!shouldTrain) throw new Error('Training not triggered');
      return apiService.trainHyperspectralModel();
    },
    [shouldTrain]
  );
...
// Real-time hyperspectral data hook
export function useRealTimeHyperspectralPredictions(intervalMs: number = 60000): ApiHook<HyperspectralPredictionsResponse> {
  const hyperspectralHook = useHyperspectralPredictions();
  
  useEffect(() => {
    if (intervalMs <= 0) return;
    
    const interval = setInterval(() => {
      hyperspectralHook.refetch();
    }, intervalMs);
    
    return () => clearInterval(interval);
  }, [hyperspectralHook.refetch, intervalMs]);
  
  return hyperspectralHook;
}
